...
import { PoemProjectRequired, PoemWriter } from "@/components/poem-writer";
...
const navigation: { label: string; icon: IconName; section?: "projects" | "poem" | "guidelines" }[] = [
  { label: "프로젝트", icon: "folder", section: "projects" },
  { label: "시 작성", icon: "pen", section: "poem" },
  { label: "시화", icon: "image" },
...
export function StudioShell() {
  ...
  const [section, setSection] = useState<"projects" | "poem" | "guidelines">("projects");
  ...
  const selectedProject = database.allProjects.find((project) => project.id === selectedId) ?? null;
  const poemProject = selectedProject ?? database.allProjects[0] ?? null;
  ...
  function changeSection(nextSection: "projects" | "poem" | "guidelines") {
    if (section === "poem" && nextSection !== "poem" && poemDirty) {
      const leave = window.confirm("저장되지 않은 변경이 있습니다. 페이지를 이동할까요?");
      if (!leave) return;
    }
    setPoemDirty(false);
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa] text-[#202124] transition-colors dark:bg-[#08090a] dark:text-[#f7f8f8] lg:grid lg:grid-cols-[220px_minmax(0,1fr)_320px] lg:grid-rows-[72px_minmax(0,1fr)]">
        ...
        <nav aria-label="주 메뉴">
          ...
          <ul className="grid grid-cols-2 gap-1 sm:grid-cols-3 lg:block lg:space-y-1">
            {navigation.map((item) => {
              ...
              return <li key={item.label}><button type="button" onClick={() => item.section && changeSection(item.section)} disabled={!item.section || !database.ready} aria-current={active ? "page" : undefined} className={`flex min-h-11 w-full items-center gap-2.5 rounded-lg px-3 text-left text-[13px] font-medium lg:min-h-10 ${active ? "bg-[#eeefff] text-[#4f57bb] dark:bg-[#5e6ad2]/20 dark:text-[#aeb4ff]" : item.section ? "text-[#686d74] hover:bg-[#f5f6f8] dark:text-[#a0a5ad] dark:hover:bg-white/[0.04]" : "cursor-not-allowed text-[#a3a7ad] dark:text-[#555960]"}`}><Icon name={item.icon} className="h-[18px] w-[18px] shrink-0" /><span>{item.label}</span>{!item.section && <span className="ml-auto hidden text-[9px] lg:inline">준비중</span>}</button></li>;
            })}
          </ul>
        </nav>
        <div className="mt-5 hidden rounded-xl border border-black/8 bg-[#fafafa] p-3 dark:border-white/8 dark:bg-white/[0.025] lg:block"><p className="text-xs font-medium">{section === "guidelines" ? "지침 관리 단계" : section === "poem" ? "시 작성 단계" : "프로젝트 관리 단계"}</p><p className="mt-1 text-[11px] leading-5 text-[#7a7f87] dark:text-[#70757d]">{section === "guidelines" ? "6종 제작 지침과 버전을 관리합니다." : section === "poem" ? "시 초안과 편집 버전을 관리합니다." : "현재는 프로젝트 정보만 관리합니다."}</p></div>
      </aside>
      ...
      <header className="flex min-h-[72px] items-center justify-between gap-4 border-b border-black/8 bg-white/90 px-5 backdrop-blur dark:border-white/8 dark:bg-[#08090a]/90 sm:px-7 lg:col-span-2">
        <div className="min-w-0"><p className="text-[11px] font-medium text-[#898d94] dark:text-[#62666d]">{section === "guidelines" ? "지침 관리" : section === "poem" ? "시 작성" : "프로젝트 관리"}</p><h1 className="truncate text-base font-semibold tracking-tight sm:text-lg">{section === "guidelines" ? "제작 지침" : section === "poem" ? "시 편집 스튜디오" : selectedProject?.name ?? "내 프로젝트"}</h1></div>
      ...
      </> : section === "poem" ? poemProject ? <PoemWriter key={poemProject.id} database={database} projectId={poemProject.id} projectName={poemProject.name} onDirtyChange={setPoemDirty} /> : <PoemProjectRequired onCreate={() => setCreateOpen(true)} /> : <GuidelineManager database={database} />}
